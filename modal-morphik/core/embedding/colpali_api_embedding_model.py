...
class ColpaliApiEmbeddingModel(BaseEmbeddingModel):
    def __init__(self):
        ...
        self.endpoint = f"{domain.rstrip('/')}/process_colpali"
    ...
    async def call_api(self, inputs, input_type) -> List[MultiVector]:
        ...
        # Process inputs one by one for Modal endpoint
        all_embeddings = []
        for input_data in inputs:
            if input_type == "image":
                payload = {"type": "image", "data": input_data}
            else:
                payload = {"type": "text", "data": input_data}
            ...
            async with AsyncClient(timeout=timeout) as client:
                ...
                if result.get("status") == "success":
                    ...
                    logger.debug(f"Received embeddings for {input_type}, shape: {result.get('shape')}")
                else:
                    logger.error(f"Error from ColPali API: {result.get('error')}")
                    raise RuntimeError(f"ColPali API error: {result.get('error')}")
