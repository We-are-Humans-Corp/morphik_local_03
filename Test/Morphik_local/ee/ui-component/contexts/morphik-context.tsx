...
export function MorphikProvider({
  ...
  const [manualAuthToken, setManualAuthToken] = useState<string | null>(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (storedUser) {
        try {
          ...
          // Ensure we have username as name for display
          if (user) {
            return {
  ...
  const authToken = manualAuthToken || connectionInfo?.authToken || null;
