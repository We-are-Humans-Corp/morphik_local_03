...
export function ModelManager({ apiKeys, authToken }: ModelManagerProps) {
  ...
  const handleAddModel = async () => {
    ...
    try {
      ...
      if (authToken) {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/models`, {
          method: "POST",
          headers: {
  ...
  const handleDeleteModel = async (id: string) => {
    try {
      if (authToken) {
        // Delete from backend
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/models/${id}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
