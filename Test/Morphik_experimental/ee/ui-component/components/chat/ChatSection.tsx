...
const ChatSection: React.FC<ChatSectionProps> = ({
  ...
  // Selected chat ID – restore from localStorage or start fresh
  const [chatId, setChatId] = useState<string>(() => {
    // Try to restore last active chat from localStorage
    if (typeof window !== 'undefined') {
      const storedChatId = localStorage.getItem('morphik-active-chat-id');
      if (storedChatId) {
        return storedChatId;
      }
    }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && chatId) {
      localStorage.setItem('morphik-active-chat-id', chatId);
    }
  }, [chatId]);
